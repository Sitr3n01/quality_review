...
- `package.json` scripts (`quality:report`, `quality:check`, `quality:baseline`)
...
8. complexity (heuristic in v1; ratchet)
...
- large PR
- baseline missing
- complexity analysis being heuristic
...
For Unity projects, see `references/unity-extension.md`. The v1 ships heuristics suitable for Node/TS/JS; Unity-specific rules are documented as future work but the architecture leaves room.
