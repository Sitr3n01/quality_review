...
# install-or-sync.sh
#
# Mirror the lucas-quality-gate skill files between .claude/skills and .agents/skills.
#
# Use cases:
...
while [ "$REPO_ROOT" != "/" ]; do
  if [ -d "$REPO_ROOT/.claude/skills/lucas-quality-gate" ] || [ -d "$REPO_ROOT/.agents/skills/lucas-quality-gate" ]; then
    break
  fi
  REPO_ROOT="$(dirname "$REPO_ROOT")"
done

CLAUDE_DIR="$REPO_ROOT/.claude/skills/lucas-quality-gate"
AGENTS_DIR="$REPO_ROOT/.agents/skills/lucas-quality-gate"
